      ...
      memcached.get(visit.uKey, function(err, result_id){
         ...
         if (result_id) {
            db.collection('visits', function(err, collection){
               ...
               collection.update({_id: ObjectID(result_id)}, visit, /*upsert*/ true, function(err, result){       // upsert - that is, if the record(s) do not exist, insert one. Upsert only inserts a single document.
                  if (err) console.log(clc.red(err.message));
               });
            });
